...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Dodanie eventów do przycisków otwierających modal
    ...
    // Kliknięcie w kartę projektu także otwiera modal
    document.querySelectorAll('.project-card').forEach(card => {
        card.addEventListener('click', function(event) {
            // Pomijamy, jeśli kliknięto w przycisk (tam już jest event)
            if (!event.target.closest('.project-buttons')) {
                const slug = this.getAttribute('data-project-slug');
                if (slug) {
                    openProjectModal(slug);
                }
